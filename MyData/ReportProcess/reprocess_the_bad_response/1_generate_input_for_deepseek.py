"""
Need another round because LLM will sometimes generate bad response: empty string.
already get all the bad response id.
"""
"""
...
def extractReport(file_path, bad_id_path):
    """
    ...
    with open(file_path, 'r') as f:
        reports = json.load(f)
    with open(bad_id_path, 'r') as f:
        bad_ids = f.readlines()
    bad_ids = [bad_id.strip() for bad_id in bad_ids]
    bad_ids = set(bad_ids)
    print(len(bad_ids))

    for report in reports:
        cur_id = report['id']
        if cur_id not in bad_ids:
            continue
...
if __name__ == '__main__':
    ...
    id_report_json = extractReport(
        ...
        f"/data/sc159/data/MIMIC_III/llava_rad_topic/chat_{split}_MIMIC_CXR_all_gpt4extract_rulebased_v1_ignored.json")
    print(len(id_report_json))
    ...
    for i in tqdm(range(0, len(id_report_json), interval), desc="Processing batches"):
        ...
        dest_file = f"/data/sc159/LLaVARad/MyData/ReportProcess/{system_prompt_cate}/re_for_bad_response/{split}_prompts_bad_{i}_{i + interval}.jsonl"
