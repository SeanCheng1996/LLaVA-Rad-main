...
def data_loader_mimic_topic_findings(data_path, split):
...
def data_loader_mimic_topic_withoutSeg_reason_findings(data_path, split):
    logging.info(f"using the MIMIC-CXR topic finding loader: MIMIC {split}.")
    with open(data_path) as f:
        dataset = json.load(f)
    with open(BAD_IMAGE_PATHS, 'r') as f:
...
data_loaders = {
    ...
    "iuxray_test_topic_findings": lambda x: data_loader_IUXRay_topic_findings(x, "test"),
    ...
    "mimic_topic_withoutSeg_reason_findings": lambda x: data_loader_mimic_topic_withoutSeg_reason_findings(x, "train"),
